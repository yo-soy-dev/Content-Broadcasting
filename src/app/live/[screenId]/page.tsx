...
function ErrorScreen() {
  return (
    <div className="h-screen w-full bg-gray-950 flex flex-col items-center justify-center gap-6">
      <div className="w-20 h-20 rounded-2xl bg-red-500/10 border border-red-500/20 flex items-center justify-center">
        <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-red-400/50">
      ...
      <button
        ...
        className="px-4 py-2 text-xs font-semibold rounded-xl bg-white/5 border border-white/10 text-white/50 hover:bg-white/10 hover:text-white/70 transition"
      >
        Refresh
      </button>
    </div>
  ...
  return (
    <div className="h-screen w-full bg-gray-950 flex flex-col items-center justify-center gap-6">
      <div className="w-20 h-20 rounded-2xl bg-teal-500/10 border border-teal-500/20 flex items-center justify-center">
        <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-teal-400/50">
  ...
  return (
    <div className="h-screen w-full bg-gray-950 flex flex-col overflow-hidden relative">
      ...
      <div className="absolute top-0 left-0 right-0 h-0.5 bg-white/5 z-50">
        <div className="h-full bg-teal-400 transition-all duration-100 ease-linear"
      ...
      <div className="flex-1 flex items-center justify-center px-8 pb-8">
        <div className="w-full max-w-5xl">
          <div className="relative rounded-2xl overflow-hidden border border-white/10
            ...
            <img key={current._id} src={current.fileUrl} alt={current.title}
          ...
          <div className="flex items-end justify-between">
            <div>
              ...
              {current.description && (
                <p className="text-white/40 text-sm mt-2 max-w-xl">{current.description}</p>
              )}
            </div>
            <div className="shrink-0 ml-6 bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-right">
              ...
              <p className="text-white/70 text-sm font-semibold mt-0.5">{current.screen ?? "—"}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {activeContent.length > 1 && (
        <div className="flex justify-center gap-2 pb-6">
          {activeContent.map((item, i) => (
            <button key={item._id} onClick={() => { setIndex(i); setProgress(0); }}
              className={`rounded-full transition-all duration-300
                ${i === index ? "w-6 h-2 bg-teal-400" : "w-2 h-2 bg-white/20 hover:bg-white/40"}`} />
          ))}
        </div>
      )}
    </div>
